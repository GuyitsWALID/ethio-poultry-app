...
export function operatingDayLockBoundary(now=new Date(),lockTime="10:00:00"){
  const addis=addisParts(now);const [hour,minute]=lockTime.split(":").map(Number);const daysBack=addis.minutes>=hour*60+minute?1:2;const date=new Date(`${addis.date}T00:00:00Z`);date.setUTCDate(date.getUTCDate()-daysBack);return date.toISOString().slice(0,10);
}
