...
export default function AlertsPage() {
  ...
  const dialogTitle = dialog?.command === "submit_resolution" ? "Confirm task completion" : dialog?.command === "start" ? "Start investigation" : "Acknowledge responsibility";
  ...
  return <main className="space-y-5 pb-8">
    ...
    {dialog ? <div className="fixed inset-0 z-[220] grid place-items-center bg-forest-950/55 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setDialog(null); }}><section role="dialog" aria-modal="true" aria-labelledby="action-dialog-title" className="w-full max-w-lg overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-2xl"><header className="flex items-start justify-between gap-4 bg-forest-900 p-5 text-white"><div><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-amber-400">Accountable action</p><h2 id="action-dialog-title" className="mt-1 font-display text-2xl font-semibold">{dialogTitle}</h2><p className="mt-2 text-xs leading-5 text-sand-200">{dialog.action.title}</p></div><button type="button" onClick={() => setDialog(null)} aria-label="Close" className="grid h-9 w-9 place-items-center rounded-lg border border-white/15"><X className="h-4 w-4" /></button></header><div className="space-y-4 p-5">{dialog.command === "submit_resolution" ? <div className="rounded-xl border border-leaf-500/25 bg-leaf-500/10 p-3 text-xs leading-5 text-forest-800">Confirm that you corrected the source record. The CEO will receive a notification, review the updated source, and run the final system check.</div> : null}<label className="block text-sm font-semibold text-forest-900">{dialog.command === "submit_resolution" ? "What did you fix?" : "Work note"}<textarea autoFocus value={note} onChange={(event) => setNote(event.target.value)} rows={4} placeholder={dialog.command === "submit_resolution" ? "Briefly describe the stock, record, or value you corrected." : "Describe what you will check and the immediate next step."} className="mt-2 w-full rounded-xl border border-sand-200 p-3 text-sm font-normal" /></label>{dialog.command === "submit_resolution" ? <label className="block text-sm font-semibold text-forest-900">Reference or evidence <span className="font-normal text-forest-500">(optional)</span><textarea value={evidence} onChange={(event) => setEvidence(event.target.value)} rows={3} placeholder="Optional receipt number, count sheet, or observation." className="mt-2 w-full rounded-xl border border-sand-200 p-3 text-sm font-normal" /><span className="mt-1 block text-xs font-normal text-forest-500">Your confirmation does not close the task. The CEO must verify that the originating check is clear.</span></label> : null}</div><footer className="flex justify-end gap-2 border-t border-sand-200 p-4"><button type="button" onClick={() => setDialog(null)} className="h-10 rounded-xl border border-sand-200 px-4 text-xs font-semibold">Cancel</button><button type="button" disabled={note.trim().length < (dialog.command === "submit_resolution" ? 8 : 4) || busy === dialog.action.actionId} onClick={() => void update(dialog.action.actionId, dialog.command === "submit_resolution" ? { command: dialog.command, note, evidence } : { command: dialog.command, note })} className="h-10 rounded-xl bg-forest-900 px-4 text-xs font-semibold text-white">{dialog.command === "submit_resolution" ? "Complete and notify CEO" : "Save and continue"}</button></footer></section></div> : null}
  </main>;
}
