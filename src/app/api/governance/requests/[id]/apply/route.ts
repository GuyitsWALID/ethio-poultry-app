...
export async function POST(_:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const {id}=await params;const auth=await createClient();const {data:request}=await (auth as any).from("governance_requests").select("request_type").eq("id",id).maybeSingle();
  const rpc=request?.request_type==="egg_opening_balance"?"apply_egg_opening_balance_request":request?.request_type==="sales_unit_conversion"?"apply_sales_unit_conversion_request":"apply_governance_request";
  const {data,error}=await (auth as any).rpc(rpc,{p_request_id:id});
