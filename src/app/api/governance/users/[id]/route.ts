...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const {data,error}=await governanceAdmin.from("profiles").update({role:nextRole,is_active:nextActive}).eq("id",id).eq("org_id",ctx.orgId).select("id,role,is_active").single();if(error)return accessJson({error:error.message},400);await governanceAdmin.from("governance_audit_events").insert({org_id:ctx.orgId,actor_id:ctx.userId,actor_role:ctx.role,event_type:"tenant_user.updated",entity_table:"profiles",entity_id:id,before_values:before,after_values:data});return accessJson({profile:data});
}
