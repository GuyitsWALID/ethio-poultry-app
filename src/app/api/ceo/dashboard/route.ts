...
export async function GET(request: NextRequest) {
  try {
    ...
    const [farms, flocks, daily, priorDaily, sales, priorSales, stock, items, costs, leads, orders, targets, vaccines] = await Promise.all([
      ...
      allRows<Record<string, unknown>>((a,b) => admin.from("flocks").select("id,flock_code,flock_type,farm_id,house_id,batch_id,current_count,status,breed_id").eq("org_id", profile.org_id).range(a,b)),
    ...
    return json({
      meta:{ dateFrom,dateTo,previous,scope,refreshedAt:new Date().toISOString(),recordCoverage,calculationCompleteness:production.completeness,latestRecordAt:production.latestAt,financialStatus: estimatedCost > 0 ? "estimate" : "unavailable" },
