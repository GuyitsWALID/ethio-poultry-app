...
import { CalendarClock, CheckCircle2, Download, FileClock, Loader2, Pause, Play, Plus, RefreshCw } from "lucide-react";
...
type Center = { capabilities: { canSchedule: boolean; canGenerate: boolean }; schedules: Schedule[]; runs: Run[]; recipients: Recipient[] };
...
export function ManagementReportCenter({ scope, dateFrom, dateTo, scopeLabel }: { scope: Scope; dateFrom: string; dateTo: string; scopeLabel: string }) {
  ...
  const act = async (key: string, body: unknown) => {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : "The report request failed."); }
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-sm" aria-labelledby="saved-reports-heading">
      <div className="flex flex-col gap-4 border-b border-sand-200 bg-forest-900 p-5 text-white lg:flex-row lg:items-center lg:justify-between">
        <div><p className="text-[10px] font-semibold uppercase tracking-[.2em] text-amber-300">Management distribution</p><h2 id="saved-reports-heading" className="mt-1 font-display text-2xl font-semibold">Branded reports & schedules</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-sand-100">Freeze the current evidence into a management-ready snapshot, then download the exact report later. Scheduled reports run in Addis Ababa time.</p></div>
        <div className="flex flex-wrap gap-2"><button type="button" onClick={generate} disabled={busy !== null || !center?.capabilities.canGenerate} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-4 text-xs font-semibold text-forest-900 disabled:opacity-50">{busy === "generate" ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileClock className="h-4 w-4" />}Generate snapshot</button>{center?.capabilities.canSchedule ? <button type="button" onClick={() => setShowSchedule((value) => !value)} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/25 px-4 text-xs font-semibold hover:bg-white/10"><CalendarClock className="h-4 w-4" />Schedule report</button> : null}</div>
      </div>
      ...
      {showSchedule && center?.capabilities.canSchedule ? <div className="grid gap-4 border-b border-sand-200 bg-sand-50 p-5 lg:grid-cols-[1.2fr_.8fr_.7fr_1.3fr_auto] lg:items-end">
        ...
        <fieldset><legend className="text-xs font-semibold text-forest-700">Share with</legend><div className="mt-1 flex min-h-11 flex-wrap items-center gap-3 rounded-xl border border-sand-300 bg-white px-3">{center.recipients.map((recipient) => <label key={recipient.id} className="inline-flex items-center gap-1.5 text-xs text-forest-700"><input type="checkbox" checked={recipientIds.includes(recipient.id)} onChange={(event) => setRecipientIds((current) => event.target.checked ? [...current, recipient.id] : current.filter((id) => id !== recipient.id))} />{recipient.full_name}</label>)}</div></fieldset>
      ...
      <div className="grid lg:grid-cols-[.9fr_1.1fr]">
        ...
        <div className="p-5"><h3 className="font-semibold text-forest-900">Saved report history</h3><p className="mt-1 text-xs text-forest-500">Each completed file is an immutable snapshot of the evidence available at generation time.</p><div className="mt-4 space-y-2">{center?.runs.length ? center.runs.map((run) => <div key={run.id} className="flex flex-col gap-3 rounded-xl border border-sand-200 p-3 sm:flex-row sm:items-center"><div className={`grid h-9 w-9 shrink-0 place-items-center rounded-full ${run.status === "completed" ? "bg-leaf-500/15 text-leaf-600" : "bg-ember-500/10 text-ember-600"}`}><CheckCircle2 className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-forest-900">{run.report_name}</p><p className="mt-0.5 text-[11px] text-forest-500">{run.period_from} – {run.period_to} · {timestamp(run.generated_at ?? run.created_at)}</p>{run.failure_message ? <p className="mt-1 text-[11px] text-ember-600">{run.failure_message}</p> : null}</div>{run.status === "completed" ? <div className="flex gap-2"><a href={`/api/reports/management/${run.id}/download?format=html`} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg bg-forest-900 px-3 text-[11px] font-semibold text-white"><Download className="h-3.5 w-3.5" />Branded report</a><a href={`/api/reports/management/${run.id}/download?format=csv`} className="inline-flex min-h-9 items-center rounded-lg border border-sand-300 px-3 text-[11px] font-semibold text-forest-700">CSV</a></div> : null}</div>) : <div className="rounded-xl border border-dashed border-sand-300 p-5 text-center text-xs text-forest-500">Generate the first management snapshot for this reporting period.</div>}</div></div>
      </div>
    </section>
  );
}
