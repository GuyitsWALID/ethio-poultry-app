...
const statusLabel: Record<ActionStatus, string> = {
  open: "Unassigned", assigned: "Assigned", acknowledged: "Acknowledged", in_progress: "In progress",
  awaiting_verification: "Waiting for verification", escalated: "Escalated", resolved: "Resolved",
};
...
export function HeaderAlertBell() {
  ...
  return (
    <div ref={rootRef} className="relative z-[160]">
      ...
      {open ? (
        <section id="header-notification-panel" aria-label="Attention and notifications" className="fixed inset-x-3 top-[70px] z-[170] max-h-[calc(100vh-5rem)] overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-[0_24px_70px_rgba(29,42,31,.22)] sm:absolute sm:inset-x-auto sm:right-0 sm:top-[calc(100%+12px)] sm:w-[430px]">
          <div className="border-b border-sand-200 bg-forest-900 p-4 text-white">
            <div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-amber-300">Operations inbox</p><h2 className="mt-1 font-display text-xl font-semibold">Attention and updates</h2><p className="mt-1 text-xs text-sand-200">Unfinished work stays visible until system verification.</p></div><div className="flex gap-2"><div className="rounded-xl border border-ember-400/20 bg-ember-400/10 px-2.5 py-2 text-center"><strong className="block text-lg leading-none">{attentionCount}</strong><span className="mt-1 block text-[8px] uppercase tracking-[.1em] text-sand-200">Attention</span></div><div className="rounded-xl border border-white/10 bg-white/[.07] px-2.5 py-2 text-center"><strong className="block text-lg leading-none">{unreadCount}</strong><span className="mt-1 block text-[8px] uppercase tracking-[.1em] text-sand-200">Unread</span></div></div></div>
          </div>

          <div className="grid grid-cols-2 border-b border-sand-200 bg-sand-50 p-1.5" role="tablist" aria-label="Operations inbox sections">
            <button type="button" role="tab" aria-selected={view === "attention"} onClick={() => setView("attention")} className={`min-h-10 rounded-lg px-3 text-xs font-semibold ${view === "attention" ? "bg-white text-forest-900 shadow-sm" : "text-forest-600"}`}>Needs attention <span className="ml-1 text-ember-500">{attentionCount}</span></button>
            <button type="button" role="tab" aria-selected={view === "updates"} onClick={() => setView("updates")} className={`min-h-10 rounded-lg px-3 text-xs font-semibold ${view === "updates" ? "bg-white text-forest-900 shadow-sm" : "text-forest-600"}`}>Updates <span className="ml-1 text-amber-700">{unreadCount}</span></button>
          </div>
