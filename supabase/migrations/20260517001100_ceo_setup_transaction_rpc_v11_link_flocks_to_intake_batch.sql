-- v11: Link all created flocks to the parent intake batch for traceability.

alter table public.flocks
add column if not exists intake_batch_id uuid null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'flocks_intake_batch_id_fkey'
  ) then
    alter table public.flocks
    add constraint flocks_intake_batch_id_fkey
    foreign key (intake_batch_id)
    references public.branch_intake_batches(id)
    on delete set null;
  end if;
end $$;

create index if not exists idx_flocks_intake_batch_id
  on public.flocks(intake_batch_id);
...
begin
  ...
  loop
    ...
    loop
      ...
      loop
        ...
        if v_primary_flock_id is null then
          v_primary_farm_id := v_farm_id;
          v_primary_house_id := v_house_id;
          v_primary_flock_id := v_flock_id;
        end if;
      end loop;
    end loop;
  end loop;
  ...
  if v_primary_flock_id is null then
    raise exception 'At least one flock is required to initialize a batch';
  end if;
