-- Update transactional CEO setup RPC after initial migration has already run.
-- This migration redefines the function to include farm-location fallback behavior.
...
begin
  ...
  loop
    insert into public.farms (name, location, branch_id, org_id)
    values (
      v_farm->>'name',
      coalesce(nullif(v_farm->>'location', ''), p_branch->>'location'),
      v_branch_id,
      p_org_id
    )
    ...
    loop
      insert into public.houses (name, capacity, farm_id, org_id)
      values (
        v_house->>'name',
